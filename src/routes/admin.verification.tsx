...
import { verificationQueue, type VerificationStatus } from "@/lib/mock-data";
...
function Verification() {
  ...
  const [pendingAction, setPendingAction] = useState<{ id: string; status: VerificationStatus } | null>(null);
  const rows = queue.filter((v) => v.status === tab);
  const target = queue.find((v) => v.id === pendingAction?.id);

  const applyStatus = (id: string, status: VerificationStatus) => {
