...
import {
  ...
  createCampusDrive,
} from "@/server/colleges/service";
import { validateSessionToken, SESSION_COOKIE_NAME } from "@/server/auth/session";
import { requireCollege } from "@/server/auth/rbac";
import { CollegeStudentCreateSchema, CampusDriveCreateSchema } from "@/server/shared/validation";
...
export const getCollegeCampusDrivesFn = createServerFn({ method: "GET" }).handler(async () => {
  try {
    ...
    return { success: true as const, data: drives };
  } catch (error) {
    return formatErrorResponse(error);
  }
});
...
export const createCampusDriveFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      return { success: true as const, data: drive };
    } catch (error) {
      return formatErrorResponse(error);
    }
  });
