...
import { signup, login, logout, registerOrganization } from "@/server/auth/service";
import { validateSessionToken, SESSION_COOKIE_NAME, SESSION_MAX_AGE_SECONDS } from "@/server/auth/session";
import { SignupSchema, LoginSchema, OrgRegistrationSchema } from "@/server/shared/validation";
import { formatErrorResponse } from "@/server/shared/errors";

export const getSessionFn = createServerFn({ method: "GET" }).handler(async () => {
  try {
    const token = getCookie(SESSION_COOKIE_NAME);
    if (!token) return { success: true as const, session: null };
